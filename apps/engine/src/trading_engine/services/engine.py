...
class TradingEngine:
    ...
    def _apply_commands(self) -> bool | None:
        symbol_valid: bool | None = None
        while not self._commands.empty():
            command, value = self._commands.get_nowait()
            if command == "config":
                self._config = value
                try:
                    self._gateway.select_symbol(value.symbol)
    ...
    def _run(self) -> None:
        try:
            ...
            self._event(
                "engine_status",
                {"running": False, "connected": True, "symbol_ready": False, **connection},
            )
            while not self._stop.is_set():
                try:
                    self._gateway.select_symbol(self._config.symbol)
                except MT5Error as exc:
                    self._event("engine_error", {"message": str(exc)})
                    self._event(
                        "engine_status",
                    ...
                    while not self._stop.is_set():
                        if self._apply_commands() is True:
                            break
                        time.sleep(0.05)
                    continue
                ...
                if self._gateway.open_strategy_positions(config.symbol, config.magic_number):
                    self._pattern.lock(
                ...
                reinitialize = False
                while not self._stop.is_set() and not reinitialize:
                    symbol_valid = self._apply_commands()
                    if symbol_valid is not None:
                        reinitialize = True
                        continue
                    config = self._config
                    ...
                    if tick.time_msc // 1000 >= last_bar + timeframe_seconds(config.timeframe):
                        ...
                        for signal in signals:
                            ...
                            try:
                                ...
                                self._event(
                                    "order_rejected",
                                    {"message": str(exc), "leg": intent.leg},
                                )
                            else:
                                ...
                                if config.stop_after_entry:
                                    ...
                                    self._event(
                                        "config_applied", stopped.model_dump(mode="json")
                                    )
                    time.sleep(config.poll_interval_ms / 1000.0)
            ...
            self._event(
                "engine_status",
                {"running": False, "connected": False, "symbol_ready": False},
            )
